...
- Possible for `fatal error: concurrent map writes` to occur. This is due to possibly multiple `goroutine` writing to the map. As such, we need to spot race conditions with the built in `race detector`.
